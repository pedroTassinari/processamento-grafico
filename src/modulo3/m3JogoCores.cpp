...
int encerrarJogo(int codigoTermino);
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        if (todosEliminados())
        {
            // Se todos os quadrados foram eliminados, encerra o jogo
            encerrarJogo(3);
            break;
        }
	}
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
        ...
        {
            // Se o quadrado já foi eliminado, encerra o jgo
            encerrarJogo(1);
            return;
        }
...
void eliminarSimilares(float tolerancia)
{
	...
	for (int i = 0; i < ROWS; i++)
	{
		for (int j=0; j < COLS; j++)
		{
			...
			float dd = d/dMax;
			if (dd <= tolerancia && !grid[i][j].eliminated && i != y && j != x)
            {
    ...
    if (!possuiSimilar)
    {
        // Se não houver quadrados similares, encerra o jogo
        encerrarJogo(2);
        return;
    }
}
...
int encerrarJogo(int codigoTermino) {
    switch (codigoTermino) {
        case 1:
            cout << "Você clicou fora dos quadrados permitidos!" << endl;
            break;
        case 2:
            cout << "Você removeu um quadrado sem cores similares" << endl;
            break;
        case 3:
            cout << "Você zerou o jogo!" << endl;
            break;
        default:
            cout << "Evento desconhecido!" << endl;
    }
    cout << "Fim de jogo! Você fez " << pontos << " pontos!" << endl;
